...
router.post('/api/send-email', requireAuth, verifyCsrf, async (req, res) => {
  ...
  // Procesar attachments opcionales proporcionados como rutas relativas al repo
  // Aceptamos: { attachments: ["path/to/file.xlsx", { path: "path/to/otro" }] }
  let attachments = []
  try {
    const raw = req.body && req.body.attachments
    if (raw) {
      if (!Array.isArray(raw)) throw new Error('attachments must be array')
      for (const item of raw) {
        const rel = (typeof item === 'string') ? item : (item && item.path)
        if (!rel) continue
        // Normalizar y resolver ruta absoluta
        const abs = path.resolve(process.cwd(), rel)
        // Evitar rutas fuera del workspace
        const relCheck = path.relative(process.cwd(), abs)
        if (relCheck.startsWith('..')) {
          return res.status(400).json({ error: 'invalid attachment path' })
        }
        if (!fs.existsSync(abs)) {
          return res.status(400).json({ error: 'attachment not found', path: rel })
        }
        attachments.push({ filename: path.basename(abs), path: abs })
      }
    }
  } catch (attErr) {
    return res.status(400).json({ error: 'invalid attachments', detail: String(attErr) })
  }
